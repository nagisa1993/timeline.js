...
var timeline = function () {
	...
	_createClass(timeline, [{
		...
		value: function draw(data) {
			...
			var clipPath = svg.append("defs").append("clipPath").attr("id", "display").append("rect").attr("x", 0).attr("y", 0).attr("width", 800).attr("height", 100).style("fill", "none");
			...
			.each(function (alignment, i) {
				...
				d3.select(this).selectAll("path").data(alignment.value).enter().append("path").attr("stroke", "rgb(102,133,194)") // 颜色可以从colormap里取
				.attr("stroke-width", 5).attr("d", function (activity, j) {
			...
			mask = group.append("rect").attr("class", "navigator-mask").style("fill", "rgba(102,133,194,0.3)").attr("transform", "translate(0,100)").style("cursor", "pointer").attr("width", 100).attr("height", 50).on("mousedown", selectElement);

			leftControl = group.append("rect").attr("class", "mask-controller").attr("stroke", "#cccccc").attr("stroke-width", 1).style("fill", "#e6e6e6").attr("width", 5).attr("height", 15).style("cursor", "pointer").attr("transform", "translate(-2.5,115)").on("mousedown", selectElement);

			rightControl = group.append("rect").attr("class", "mask-controller").attr("stroke", "#cccccc").attr("stroke-width", 1).style("fill", "#e6e6e6").attr("width", 5).attr("height", 15).style("cursor", "pointer").attr("transform", "translate(97.5,115)").on("mousedown", selectElement);
		}
	}]);
